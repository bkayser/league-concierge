...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if (isLoggingEnabled()) {
    try {
      ...
      console.error("Failed to fetch sources from database:", err);
      return Response.json({ error: "Database unavailable" }, { status: 502 });
    }
  }
  ...
  try {
    ...
    console.error("Failed to fetch documents from Pinecone:", err);
    return Response.json({ error: "Vector store unavailable" }, { status: 502 });
  }
}
